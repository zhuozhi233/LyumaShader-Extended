...
- 可选“剖面跟随相机”，让 2D 剖面同时跟随相机的水平与俯仰方向并保持与画面平行；默认关闭以保留旧配置效果。
- 可添加多个自定义游戏菜单项，分别设置名称、图标、菜单位置、参数名以及默认启用、保存和同步状态；指定 Sub Menu 时创建到其内部，其他 MA Menu Item 可直接复用。
...
- Modular Avatar `1.13.0` 或更高版本。
- NDMF `1.8.0` 或更高版本。

以上是最低版本要求，不是固定版本要求；满足最低版本的更高兼容版本可以正常安装。本包不单独声明 VRCSDK 的版本要求。
...
- lilToon 与 Poiyomi 不强制限定为测试环境中的具体版本；Modular Avatar 与 NDMF 只要求满足上述最低版本。
